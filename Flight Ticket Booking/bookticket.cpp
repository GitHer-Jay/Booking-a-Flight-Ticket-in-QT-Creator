...
//aeroplane * bookticket::ptrToAero = nullptr;
...
void bookticket::getTicketsFromDB()
{
    ...
    QSqlDatabase  db =  DBUtility::getInstance()->getDBInstance();
    QSqlQuery query(db);
        query.prepare("select seat_number from cppbuzz_aero_seats where available='Y'");
...
void bookticket::setAeroplane(aeroplane *ptr)
{
    qDebug()<<" bookticket::setAeroplane()";
    this->ptrToAero = ptr;
}
...
void bookticket::checkStatus()
{
    ...
    if(ptrToAero == nullptr)
        qDebug() << "ptrToAero is null";
...
void bookticket::reset()
{
    ...
    QSqlDatabase  db =  DBUtility::getInstance()->getDBInstance();
    QSqlQuery query(db);
        query.prepare("update cppbuzz_aero_seats set available='Y'");
